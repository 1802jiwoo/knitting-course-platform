...
public class LectureService {
    ...
    // GET /api/lectures — APPROVED만 공개
    public List<LectureListResponse> getLectures(String keyword) {
        return lectureRepository.searchApproved(keyword)
                .stream()
                .map(l -> LectureListResponse.from(l, enrollmentRepository.countByLectureId(l.getLectureId())))
                .toList();
    }
